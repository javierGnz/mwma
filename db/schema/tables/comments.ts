...
export const commentsTable = pgTable("comments", {
	...
	work_id: uuid("work_id").references(() => worksTable.id, { onDelete: "cascade" }),
	task_id: uuid("task_id").references(() => tasksTable.id, { onDelete: "cascade" }),
...
export const commentRelations = relations(commentsTable, ({ one }) => ({
	work: one(worksTable, { fields: [commentsTable.work_id], references: [worksTable.id] }),
	task: one(tasksTable, { fields: [commentsTable.task_id], references: [tasksTable.id] }),
